...
//*****************************************************************************
// Version: 1.1.0 (patched: persistOrder flag, dragtable:columnMoved event)
var dragtableVersion = "1.1.0";

dragtable = {
  // Whether column-drag order should be remembered across page loads via
  // cookies. Set to true to restore the original dragtable behavior.
  persistOrder: false,
  ...
  makeDraggable: function(table) {
    ...
    for (var i = 0; i < headers.length; i++) {
      headers[i].onmousedown = dragtable.dragStart;
		...
		if (dragtable.persistOrder && dragtable.cookiesEnabled() && table.id &&
 ...
 absolutePosition: function(elt, stopAtRelative) {
  ...
  dragEnd: function(event) {
    ...
    if (targetCol != -1 && targetCol != dragObj.startCol) {
      ...
      if (dragtable.persistOrder && dragObj.table.id && dragtable.cookiesEnabled() &&
  ...
  moveColumn: function(table, sIdx, fIdx) {
    ...
    // Let other scripts (e.g. toolbar.js) know a column moved. They should
    // track columns by a stable identifier (data-col-key), not by index, but
    // still may want to react -- e.g. to re-sync the toolbar's left-to-right
    // checkbox order with the table's new visual column order.
    if (typeof CustomEvent === "function") {
      table.dispatchEvent(new CustomEvent("dragtable:columnMoved", {
        detail: { fromIndex: sIdx, toIndex: fIdx }
      }));
    }
  },
